...
async def diologue_stream(dialogue: Dialogue):
    try:
        return StreamingResponse(
            llm_service.chat_stream(dialogue.model, dialogue.question),
...
async def stream_chat(
    model: str = "qwen3-max", question: str = "请非常详细地介绍什么叫机器学习"
):
    try:
        return StreamingResponse(
            llm_service.chat_stream(model, question), media_type="text/event-stream"
        )
    except Exception as e:
        return {"error": str(e)}
...
@router.post("/tools")
async def tools(dialogue: Dialogue):
    try:
        return llm_service.chat_with_tools(dialogue.model, dialogue.question)
    except Exception as e:
        return {"error": str(e)}
